...
class MapViewModelTest {
    ...
    private val saveMapPointUseCase: SaveMapPointUseCase = mock()

    private val viewModel = MapViewModel(getMapPointsUseCase, saveMapPointUseCase)
    ...
    @Test
    fun `savePoint EXPECT saveMapPointUseCase is invoked`() = runTest {
        val mockPoint = MapPoint(id = 1, latitude = 59.121, longitude = 33.123)
        viewModel.savePoint(mockPoint)

        verify(saveMapPointUseCase).invoke(mockPoint)
    }
    ...
    @Test
    fun `savePoint with unknown error EXPECT error state`() = runTest {
        val mockPoint = MapPoint(id = 0, latitude = 59.121, longitude = 33.123)
        val errorMessage = "Couldn't save point"
        whenever(saveMapPointUseCase.invoke(mockPoint)) doThrow IllegalStateException(errorMessage)
